...
**NLP Processing**: We employed the Stanza NLP library (version 1.5+) with pre-trained models for Latin ('la') and Spanish ('es'). Latin texts were processed using the Latin Internet Tree Bank (ITTB) model; Spanish texts used the AnCora treebank model.
...
- **Fisher's exact tests** for categorical data (synthetic vs. analytical constructions)
- **Bootstrap confidence intervals** (n=1000) for robust uncertainty estimation
- **Cohen's d effect sizes** for magnitude assessment

Statistical significance was set at α = 0.05. While multiple comparisons were performed, key findings remain significant even under Bonferroni correction (α = 0.017). Effect sizes were not calculated for comparisons where one group showed zero variance, as this yields undefined or infinite values. All analyses were conducted in Python using SciPy (version 1.9+).
...
Analysis revealed a complete structural transition from synthetic to analytical constructions (Fisher's exact test, p = 0.0001), demonstrating systematic functional replacement across the Latin-to-Spanish transition.
...
**Statistical Considerations**: Multiple comparisons were performed without family-wise error rate corrections, potentially inflating Type I error probability. Future analyses should apply appropriate multiple testing adjustments.
...
*Data availability: Analysis code and processed results available at [repository URL]*
